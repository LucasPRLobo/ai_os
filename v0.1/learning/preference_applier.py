...
from models.suggestions import SuggestionResponse, Suggestion
...
def apply_preferences(
    ...
    for _, _, sugg, new_conf in scored_suggestions:
        # Create new suggestion with updated confidence
        updated = Suggestion(
...
def _apply_folder_names(sugg: Suggestion, store: PreferenceStore) -> Suggestion:
    ...
    Note: This modifies folder names in-place for simplicity.
    In production, you'd create new objects.
    """
    # Map of default names to scene types
    default_to_scene = {
        "selfies": "selfie",
        "beach & pool": "beach",
    ...
    for folder in sugg.folder_structure.folders:
        ...
        # Check if this matches a known scene type
