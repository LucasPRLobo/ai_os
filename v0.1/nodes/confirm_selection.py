...
Presents suggestions to the user and allows them to:
...
from models.suggestions import Suggestion, SuggestionResponse
...
def _display_suggestion(index: int, suggestion: Suggestion):
    ...
    print(f"\n{conf_icon} Option {index}: {fs.base_path}")
    print(f"   Confidence: {confidence:.0%} ({conf_label})")
...
def _get_user_choice(num_options: int) -> Optional[int]:
    ...
    Returns:
        Selected option number (1-based) or None if cancelled
    """
    print("\n" + "-" * 60)
