...
state = create_initial_state(
    input_paths=["/tmp/test_directory"],
    recursive=True
)
state = validate_input(state)
state = scan_files(state)

print(f"Found {state['total_files_scanned']} files")
print(f"Total size: {_format_size(state['total_size_bytes'])}")
print(f"Warnings: {len(state['warnings'])}")
if state['file_paths']:
    print(f"First file: {state['file_paths'][0]}")
"""