...
This is the "act" step in the flow:
...
from models.suggestions import Suggestion, FolderStructure
...
def execute_organization(state: OrganizerState) -> OrganizerState:
    ...
    use_copy = state.get("use_copy", False)  # Copy instead of move
    output_dir = state.get("output_dir")  # Optional custom output directory
    
    # Determine base output path
    if output_dir:
        base_path = Path(output_dir) / selected.folder_structure.base_path
    else:
        # Use the common parent of input files
        if files:
...
def _build_operations(
    ...
    Args:
        folder_structure: The folder structure to create
        file_lookup: Mapping of filename -> full path
...
def _confirm_execution(num_files: int) -> bool:
    ...
    print(f"\n⚠️  This will move {num_files} files to new locations.")
...
def _execute_operations(
    ...
    Returns:
        Result dict with status, counts, and any errors
    """
    if dry_run:
        return {
            "status": "dry_run",
    ...
    for source, dest in operations:
        try:
            ...
            error_count += 1
            print(f"   ✗ {Path(source).name} (permission denied)")
        except Exception as e:
            errors.append(f"Error with {source}: {str(e)}")
            error_count += 1
            print(f"   ✗ {Path(source).name} ({str(e)})")
...
def dry_run_organization(state: OrganizerState) -> OrganizerState:
    ...
    Sets dry_run flag and calls execute_organization.
    """
    state["dry_run"] = True
