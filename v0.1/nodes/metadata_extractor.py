...
def _extract_file_metadata(
    ...
    return FileMetadata(
        ...
        hash=None  # Could compute hash here if needed
    )
...
# Check results
print(f"Extracted metadata for {len(state['files'])} files")
if state['files']:
    file = state['files'][0]
    print(f"\\nFirst file:")
    print(f"  Name: {file.name}")
    print(f"  Type: {file.content_type}")
    print(f"  Size: {file.get_size_human()}")
    print(f"  Preview: {file.content_preview[:100] if file.content_preview else 'None'}...")
"""